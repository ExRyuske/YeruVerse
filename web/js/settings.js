...
function migrate(values) {
  // Эхоподавления в приложении больше нет — вместе с настройкой уходит и
  // сохранённое значение, иначе оно осталось бы в хранилище навсегда.
...
export class Settings extends Emitter {
   ...
   *
   * Ни трансляция, ни звук собеседников сюда не заглядывают: у демонстрации
   * экрана свои ограничения (см. `shares.js`), а у воспроизведения их не бывает
   * вовсе. Настройка микрофона не должна быть слышна ни в чужом звуке, ни в
   * своём стриме.
   *
   * Браузерное подавление включаем только когда выбрано именно оно: вместе с
   * RNNoise два шумодава подряд съедают согласные и делают голос ватным.
   *
   * Эхоподавление выключено всегда и настройкой больше не является. Ради него
   * движок берёт микрофон в «разговорном» режиме: полоса режется до телефонной,
   * громкость гуляет, а всё, что похоже на отражение, — второй голос в комнате,
   * музыка, свой же звук из динамиков — глушится вместе с эхом. Просить его
   * явным `false` приходится потому, что без просьбы движок включает его сам.
   */
  micConstraints() {
    const c = {
