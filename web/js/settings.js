...
const DEFAULTS = {
  ...
  // Автоматический gate: пока не говорите, микрофон в эфир не уходит. Порог
  // тот же, что у индикатора «говорит» — настраивать его отдельно нечем же,
  // чем и сам индикатор, поэтому выключатель один на двоих.
  gate: true,
  monitor: false,     // слышать себя — только в наушниках, иначе заведётся
  // Не отдавать звук системе в «разговорном» режиме. Только Android и
  // только в приложении; в браузере делать с этим нечего. Включено по
  // умолчанию: узкая полоса слышна всем и сразу, а обратный случай —
  // когда правка режима мешает, — пока гипотетический.
  wideband: true,
...
export class Settings extends Emitter {
  ...
  /**
   * Забыть id участника, покинувшего комнату. Id выдаются заново на каждый
   * вход и никогда не повторяются — без этого `names`/`peerVolume` копились
   * бы всю жизнь вкладки, в бойкой комнате разрастаясь без предела.
   * Сохранённая по нику громкость (`byName`) не трогается: она и остаётся
   * тем самым единственным постоянным ключом.
   */
  untrackPeer(id) {
    this.names.delete(id);
    this.peerVolume.delete(id);
  }
