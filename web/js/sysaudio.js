// Звук компьютера в свою трансляцию — на macOS и на Linux.
//
// На Windows это делает сам движок: `getDisplayMedia` отдаёт звук вместе с
// картинкой, и здесь ничего не нужно. На macOS системный звук не отдаёт ни
// один движок — ни WebKit, ни Chrome; на Linux вебвью это WebKitGTK, и там то
// же самое. В обоих случаях трансляция уходит немой, и закрыть эту дыру может
// только оболочка (`sysaudio.rs`): она снимает звук — через ScreenCaptureKit
// на macOS, через звуковой сервер на Linux — и присылает сюда отсчёты.
//
 ...
 *
 * Нужно для одной-единственной проверки, зато решающей. Захват шлёт посылки
 * постоянно, даже когда в системе полная тишина: замерено — за пять секунд
 * молчания приходит ровно пять секунд нулевых отсчётов. Так ведут себя оба
 * источника: и ScreenCaptureKit, и монитор устройства вывода на Linux. Значит
 * ноль байт означает не «нечего транслировать», а «захват не работает», и об
 * этом можно честно сказать человеку, не заставляя его гадать.
 */
