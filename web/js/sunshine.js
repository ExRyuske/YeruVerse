...
mesh.on('message', async ({ id, msg }) => {
  ...
  if (msg.host) {
    sunshineHosts.set(id, { host: msg.host, canPair: !!msg.canPair, open: !!msg.open });
  }
...
function tellSunshine(id) {
  ...
  mesh.send(id, {
    ...
    // Достучится ли до нас зритель извне. Адрес мы отдаём в любом случае — в
    // одной сети с нами он рабочий, — но знать, что снаружи он не отвечает,
    // зритель должен до запуска Moonlight, а не после его отказа.
    open: mine && state.sunshineOpen,
  });
}
...
export function sunshineHint() {
  ...
  return state.sunshineOpen
    ...
    : 'Sunshine виден только в вашей сети. Самый простой путь — включить UPnP в ' +
        'самом Sunshine (Configuration → Network), тогда порты откроются сами. ' +
        'Вручную это TCP 47984, 47989 и 48010 плюс UDP 47998–48000 — веб-панель ' +
        '47990 наружу не отдавайте, за ней настройки вашего компьютера. ' +
        'За CGNAT провайдера не поможет и проброс: там остаётся Tailscale или ' +
        'ZeroTier — с ними открывать порты не нужно вовсе.';
}
 ...
 */
/**
 * Адреса, которые живут только внутри чьей-то сети. Снаружи по такому не
 * достучаться никогда: он есть у каждого второго дома и означает у всех разное.
 */
...
/**
 * Сказать заранее то, что иначе скажет Moonlight — и скажет непонятно.
 *
 * «Failed to connect to 192.168.2.212» выглядит как поломка Moonlight, хотя на
 * деле это исправная работа: адрес принадлежит домашней сети хозяина, и из
 * другой сети он не значит ничего. Мы знаем об этом заранее — хозяин сам
 * сказал, видно ли его снаружи, — и молчать об этом до отказа незачем.
 *
 * Запуск при этом не отменяем: в одной сети с хозяином такой адрес рабочий, и
 * решать за человека, где он находится, мы не можем.
 */
function warnIfUnreachable(entry) {
  if (entry.open || !isLocalAddress(entry.host)) return;
  toast(
    `${entry.host} — домашний адрес хозяина, снаружи его сети он не отвечает. ` +
      'Если вы не в одной сети с ним, попросите включить UPnP в Sunshine ' +
      '(Configuration → Network) или соединитесь через Tailscale.',
    14000
  );
}
