...
net.on('presence', ({ peer }) => {
  ...
  // Снимаем по присутствию только чужое.
  //
  // Про свои трансляции знает `state.shares`, и знает точно: там лежит живой
  // захват, и кнопка внизу показывает именно его. Присутствие же ходит кругом
  // через сервер и всегда отстаёт — а после обрыва отстаёт особенно обидно:
  // карточка на сервере заводится заново, пустой, и первая же рассылка про нас
  // говорит «трансляции нет». Своё окно верило ей на слово и снимало
  // собственную плитку, не трогая ни `state.shares`, ни кнопку: захват идёт,
  // собеседники смотрят, кнопка горит — а из своего окна трансляция пропала, и
  // вернуть её нечем. Следующая, уже правильная рассылка ничего не чинит: здесь
  // умеют только снимать.
  if (mine) return render('peers');
