...
export function volumeSlider({ max, label, get, set }) {
  ...
  // Куда вернуть громкость, если её выключили кнопкой. Шкала здесь в процентах,
  // и сотня — это «как было». Пока тут стояла единица, первое же нажатие на
  // выключенном ползунке — а выключенным он приходит из сохранённых настроек —
  // возвращало один процент: человек включал звук и всё равно ничего не слышал.
  let before = 100;
  btn.onclick = () => {
