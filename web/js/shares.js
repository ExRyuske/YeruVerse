...
/**
 * Свою трансляцию после обрыва надо объявить заново — и комнате, и себе.
 *
 * Себе — потому что своя плитка и свой переключатель живут под ключом с
 * собственным id, а он после переподключения другой: сервер выдаёт id на
 * соединение, а не на человека. Под прежним ключом трансляция снимается сверкой
 * (`syncScreens`), и без переобъявления она пропала бы из собственного окна,
 * продолжая при этом идти собеседникам, — понять, что случилось, было бы
 * неоткуда.
 */
export function announceShares() {
  for (const [kind, stream] of state.shares) {
    net.send({ t: 'presence', [SHARES[kind].presence]: true });
    addScreen(viewKey(state.self.id, kind), ownDisplay(kind, stream));
  }
}

/**
 * Свой экран без звука — для собственного превью на сцене.
 *
 * Звук компьютера и так слышен из колонок напрямую; второй раз, через
 * собственную же плитку в приложении, — только эхо, а не польза. На Windows
 * он приезжает в том же потоке, что уходит собеседникам (движок отдаёт его
 * вместе с картинкой), поэтому себе показываем отдельный поток — с тем же
 * видео, но без единой звуковой дорожки, а не тот же объект целиком.
 */
function ownDisplay(kind, stream) {
  return kind === 'screen' ? new MediaStream(stream.getVideoTracks()) : stream;
}
...
export async function toggleShare(kind) {
  ...
  try {
    ...
    // На Windows звук уже в потоке — движок отдал его вместе с картинкой. На
    // macOS его ещё нет: добирает оболочка отдельным запросом.
...
async function addSystemSound(stream) {
  ...
  try {
    ...
    // В сам `stream` дорожку не дописываем — этот объект смотрит и на сцену
    // себе (см. `ownDisplay`), а мы не хотим слышать собственный звук
    // компьютера ещё раз через приложение, когда он и так идёт из колонок.
    // Собеседникам это не мешает: `mesh` раздаёт дорожку сама, ей для этого
    // не нужно, чтобы дорожка лежала в объекте потока.
