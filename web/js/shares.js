 ...
 */
/** Системы, где звук компьютера добирает оболочка, а не движок. */
const SHELL_SOUND = ['macos', 'linux'];

/**
 * Про подмешанные голоса предупреждаем один раз за загрузку страницы.
 * Трансляцию за вечер включают и выключают не раз, и повторять одно и то же
 * при каждом запуске значило бы приучить не читать.
 */
...
async function addSystemSound(stream) {
  if (!canCaptureSound()) {
    ...
    if (native.available && SHELL_SOUND.includes(native.caps.platform)) {
  ...
  try {
    ...
    // На Linux в звук компьютера попадают и голоса собеседников: снимается
    // монитор устройства вывода, а это готовая смесь всего, что звучит, —
    // включая то, что играет сам YeruVerse. Собеседники будут слышать себя
    // с задержкой, и лучше сказать об этом заранее, чем оставить их гадать.
    if (native.caps.systemAudioMixesUs && !warnedMixed) {
      warnedMixed = true;
      toast(
        'В звук компьютера на Linux попадают и голоса собеседников — '
          + 'система не даёт снять звук без своего собственного. '
          + 'Если это мешает, выключите звук в настройках трансляции.',
        12000,
      );
    }
    watchSound(stream);
  ...
  setTimeout(async () => {
    ...
    toast(
      sent > 0
        ...
        : native.caps.platform === 'linux'
          ? 'Звуковой сервер не отдаёт звук: проверьте, что вывод идёт на то же '
            + 'устройство, что выбрано в системе по умолчанию, и что установлен '
            + 'pipewire-pulse.'
