//! Звук компьютера в трансляцию — на macOS и на Linux.
//!
...
//! Просьба молча игнорируется, и трансляция уходит немой. На Linux вебвью —
//! это WebKitGTK, и там ровно то же самое: картинку портал рабочего стола
//! отдаёт, звук — нет. Дырку эту может закрыть только оболочка.
//!
//! Систем две, и у каждой своя дорога к звуку: ScreenCaptureKit на macOS,
//! клиентская часть PulseAudio на Linux. Общего у них всё остальное — счётчик
//! отправленного, остановка с подтверждением и формат посылки, — и живёт оно
//! здесь, до платформенных модулей.
//!
//! Как закрываем на macOS. ScreenCaptureKit (macOS 13+, как раз наш минимум)
//! умеет отдавать звук всей системы отдельным потоком. Мы просим у него только звук —
...
//! ниже.
//!
//! Почему посылка ограничена байтами, а не временем. У `Channel` в Tauri два
//! пути доставки: до 1024 байт — синхронный `eval`, выполняющийся ровно в том
//! порядке, в каком его позвали; крупнее — отдельный `fetch`, а его обещания
//! у разных посылок могут разрешиться не в том порядке, в каком ушли. Дорожка
//! на странице ждёт посылки строго по счёту и копит всё, что пришло не по
//! очереди (см. её `Channel`), — а если хоть одна крупная посылка потеряется
//! по дороге, счётчик замирает навсегда, и всё, что после неё, оседает в
//! памяти, не доходя до `onmessage`. Раньше здесь стоял кусок на сорок
//! миллисекунд — крупнее 1024 байт почти при любой частоте и потому всегда
//! шёл уязвимым путём; отсюда и «звук снимается, но не доходит до страницы».
//!
...
pub async fn sound_start(
    ...
    // Ручку прошлого захвата берём копией и сразу отпускаем состояние — как и
    // в `sound_stop`, а не держим на ней замок целиком. Ждать остановки
    // прошлого захвата (и подъёма нового — тоже не мгновенного) можно и
    ...
    let live = tauri::async_runtime::spawn_blocking(move || {
        ...
        //
        // Отказ здесь был прямой дорогой к беде. Страница может уйти и
        // вернуться — перезагрузкой, сменой сервера, обычным F5, — и её
        // состояние обнулится, а наше нет: поток ScreenCaptureKit остался бы
        // жить навсегда. Мало того что звук после этого не завести, так ещё и
        // экран у системы числится занятым, и следующий `getDisplayMedia` в
        // вебвью отвечает «The operation was aborted» — то есть ломается уже
        // и сама трансляция, к звуку отношения не имеющая.
        if let Some((stop, finished)) = previous {
            let _ = stop.send(());
            await_finish(&finished);
        }
        sent.store(0, Ordering::Relaxed);
        start(channel, rate, sent)
    })
    .await
...
mod imp {
    ...
    /// Верхняя граница одной посылки в байтах — см. объяснение в шапке файла
    /// про два пути доставки `Channel`. Чуть меньше самого порога (1024) на
    /// всякий случай, без плотной подгонки под чужую константу.
    ...
    fn setup(
        ...
        // Кусок в отсчётах, чередующихся по каналам: сколько укладывается в
        // `MAX_CHUNK_BYTES` — независимо от частоты, чтобы граница не
        // поплыла вместе с ней.
        const BYTES_PER_FRAME: usize = 4; // два канала по 16 бит
        let chunk = (MAX_CHUNK_BYTES / BYTES_PER_FRAME) * 2;
...
/// Звук компьютера на Linux — через клиентскую часть PulseAudio.
///
/// Зачем он здесь. Вебвью на Linux — это WebKitGTK, и системный звук в
/// `getDisplayMedia` он не отдаёт: `systemAudio: 'include'` там не делает
/// ничего, ровно как на macOS. Демонстрация экрана идёт через портал рабочего
/// стола и картинку отдаёт исправно, а звук уходит немым.
///
/// Почему Pulse, а не родной API PipeWire. У «простого» API Pulse вся работа —
/// соединиться и читать; `pw_stream` требует своего цикла событий и
/// согласования формата через SPA-поды, то есть втрое больше кода в обмен на
/// ту же дорожку. При этом на любой живой системе с PipeWire (CachyOS в том
/// числе) слой совместимости `pipewire-pulse` стоит по умолчанию и отдаёт
/// мониторы как свои — так что этот же код снимает звук и там, и на старом
/// PulseAudio, и выбирать между ними не приходится.
///
/// `@DEFAULT_MONITOR@` — не наша выдумка, а имя, которое сервер разбирает сам:
/// «монитор того устройства вывода, которое сейчас выбрано по умолчанию».
/// Спрашивать имя монитора самим значило бы завести полный асинхронный клиент
/// Pulse ради одной строки, а заодно потерять правильное поведение при смене
/// устройства вывода на ходу.
///
/// Формат просим сразу тот, в котором дорожка нужна странице: 16 бит
/// с чередованием каналов, little-endian. Ресемплирование до частоты
/// звукового графа страницы делает сам сервер — свой пересчёт здесь был бы
/// хуже во всех отношениях.
///
/// **Чего этот путь не умеет.** На macOS у ScreenCaptureKit есть
/// `excludesCurrentProcessAudio`, и наш собственный звук в захват не попадает.
/// На Linux такого рычага нет ни у Pulse, ни у PipeWire: монитор устройства
/// вывода — это готовая смесь, уже после того, как всё сложили вместе, и
/// выдернуть из неё одного участника нечем. Значит, вместе со звуком игры в
/// трансляцию попадают и голоса собеседников, и каждый из них слышит себя
/// с задержкой. Молчать об этом нельзя, поэтому `capabilities` отдаёт
/// `systemAudioMixesUs`, а страница предупреждает — см. `sysaudio.js`.
#[cfg(target_os = "linux")]
mod imp {
    use std::sync::atomic::{AtomicU64, Ordering};
    ...
    /// Верхняя граница одной посылки в байтах — см. объяснение в шапке файла
    /// про два пути доставки `Channel`. Кратно четырём: это ровно кадр стерео
    /// по 16 бит, и делить кадр между посылками нельзя — у страницы разъедутся
    /// каналы местами.
    const MAX_CHUNK_BYTES: usize = 1000;

    /// Сколько ждать, пока поток скажет, соединился он или нет.
    const WAIT: Duration = Duration::from_secs(5);

    /// Имя, которым сервер называет «монитор нынешнего устройства вывода».
    const MONITOR: &str = "@DEFAULT_MONITOR@";

    /// Соединиться с монитором вывода.
    ///
    /// Размер куска сервера задаём явно. Без него сервер берёт свой, а он на
    /// запись бывает щедрым — до десятых долей секунды, — и звук приезжал бы
    /// заметно позже картинки. Просим ровно ту порцию, которой мы и отдаём:
    /// на 48 кГц это около пяти миллисекунд, и цикл ниже успевает замечать
    /// «стоп» без отдельного будильника.
    fn connect(rate: u32) -> Result<Simple, String> {
        let spec = Spec { format: Format::S16le, channels: 2, rate };
        if !spec.is_valid() {
            return Err(format!("PulseAudio не берёт частоту {rate}"));
        }
        let attr = BufferAttr {
            maxlength: u32::MAX,
            tlength: u32::MAX,
            prebuf: u32::MAX,
            minreq: u32::MAX,
            fragsize: MAX_CHUNK_BYTES as u32,
        };
        Simple::new(
            None,        // сервер по умолчанию
            "YeruVerse", // как нас назовут в микшере
            Direction::Record,
            Some(MONITOR),
            "Звук компьютера", // как назовут саму дорожку
            &spec,
            None, // раскладка каналов по умолчанию
            Some(&attr),
        )
        .map_err(|e: PAErr| {
            format!(
                "не подключились к звуковому серверу ({e}). \
                 Нужен PipeWire со слоем совместимости pipewire-pulse \
                 (пакет pipewire-pulse) или PulseAudio"
            )
        })
    }
    ...
    pub fn start(
        ...
        let raised = Arc::clone(&finished);

        // Чтение из Pulse блокирующее, а состояние Tauri обязано быть `Send`:
        // поток целиком живёт здесь, наружу торчит только «стоп».
        std::thread::Builder::new()
            .name("sysaudio".into())
            .spawn(move || {
                // Область — чтобы соединение успело закрыться до подтверждения:
                // подтверждение обещает, что мы отпустили устройство.
                {
                    let simple = match connect(rate) {
                        Ok(s) => {
                            let _ = ready_tx.send(Ok(()));
                            s
                        }
                    ...
                    // Условие — «в канале пусто», а не «не пришло»: оборванный
                    // канал означает ушедшее состояние приложения, то есть тот
                    // же «стоп», и продолжать после него нечего.
                    while let Err(TryRecvError::Empty) = stop_rx.try_recv() {
                        if let Err(e) = simple.read(&mut buf) {
                            // Устройство вывода могли переключить или выдернуть.
                            tracing::debug!("звук системы: чтение прервалось ({e})");
                            break;
                        }
                        // Отсчёты уже в том виде, в каком их ждёт страница, —
                        // мы просили этот формат у сервера, и пересчитывать
                        // здесь нечего.
                        if channel.send(InvokeResponseBody::Raw(buf.to_vec())).is_err() {
                            // Страница ушла — дальше слать некому.
                            break;
                        }
        ...
        ready_rx.recv_timeout(WAIT).map_err(|_| "звуковой сервер не ответил".to_string())??;
        Ok(super::Live { stop: stop_tx, finished })
    }
}
...
fn start(
    ...
    // На Windows этого моста не нужно: там движок отдаёт звук системы сам,
    // прямо в `getDisplayMedia`. Страница сюда и не постучится —
    // `capabilities` про эту сборку скажет «не умею».
    Err("захват звука системы есть только на macOS и Linux".into())
}
