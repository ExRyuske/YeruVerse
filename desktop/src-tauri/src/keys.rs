...
//!
//! Где так не выходит — на macOS, пока человек не выдал разрешения, — остаётся
//! прежний путь с захватом клавиши: см. `set_hotkeys` в `lib.rs`. На Linux нет
//! и его: см. заглушку `watch` ниже.

use std::collections::HashMap;
use std::sync::mpsc::{self, Sender};
use std::sync::{Mutex, RwLock};
// Ждать здесь приходится только платформенным наблюдателям — их потоку хуков.
...
///
/// Там, где слежения нет (Linux), сочетания разбираются, но сверять их не с
/// чем: `watch()` — заглушка, и до сверки дело не доходит. Лишним при этом
/// оказывается не код, а платформа, поэтому здесь `allow`, а не `cfg`:
/// на разборе и сверке стоят тесты, и они одни на все системы.
...
impl Dispatch {
    ...
    ///
    /// Без слежения звать её некому — см. оговорку у `Combo`.
    #[cfg_attr(not(any(target_os = "windows", target_os = "macos")), allow(dead_code))]
    ...
    ///
    /// Без слежения показывать нажатия некому — см. оговорку у `Combo`.
    #[cfg_attr(not(any(target_os = "windows", target_os = "macos")), allow(dead_code))]
...
/// На Linux системных сочетаний нет вовсе, и это не лень, а два запертых
/// замка сразу.
///
/// Смотреть за клавиатурой нечем: Wayland отдаёт чужой ввод только через
/// устройства ядра, а туда обычное приложение не пускают. Забрать клавишу
/// себе — тоже нечем: раздаёт клавиатуру композитор, просят его об этом
/// порталом `GlobalShortcuts`, а прежний путь через `XGrabKey` ушёл вместе
/// с X11 (см. Cargo.toml).
///
/// Отсюда `false`, и `capabilities` вслед за ним говорит `hotkeyMode: "none"`:
/// страница не покажет настройку, которой не на что опереться.
