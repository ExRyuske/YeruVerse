...
// Событие отсюда шлёт только захват сочетаний у системы, а его нет на
// мобильных (см. `grab_hotkeys`). Слежение за клавиатурой в `keys.rs` своё
// событие шлёт само и берёт трейт себе.
#[cfg(desktop)]
use tauri::Emitter;
...
/// курсоры зрителей и дать по нему щёлкать. Оба умеет вся настольная сборка.
///
/// `systemAudio` — про звук компьютера: может ли оболочка его снять.
#[tauri::command]
fn capabilities(app: tauri::AppHandle) -> serde_json::Value {
    ...
    serde_json::json!({
        ...
        // его не отдаёт сам: на macOS. На Windows отдаёт, и мост не нужен.
...
pub fn run() {
    ...
    // Плагина системных сочетаний нет на мобильных.
    #[cfg(desktop)]
